...
public class GetCallHist {
	public static final ApiProcessingInterface<ResponseObject> PROCESS = (RequestObject requestObject, int maxMsisdn) -> {
		...
		if(!request_type.equals("call_hist")) {
...
//		}
		...
		ResponseObject responseObject = CallHistFlow.run(bodyObj);
		...
		Path path = Paths.get(cdr, requestObject.getRequest_id(), year, month, "CH-"+year+month+day+".cdr");
